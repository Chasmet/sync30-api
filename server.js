...
const VEED_MAX_SECONDS = 30;

app.get("/", (req, res) => {
  res.json({
    status: "Sync30 API active",
    maxVideo: "Kling 9 sec max / VEED 30 sec max",
    maxAudio: "Kling 9 sec max / VEED 30 sec max",
  ...
  async (req, res) => {
    try {
      ...
      const maxAllowedSeconds =
        engine === "kling" ? KLING_MAX_SECONDS : VEED_MAX_SECONDS;
